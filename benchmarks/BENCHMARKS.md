...
- **Hybrid（+ BGE-M3）**：在 builtin 评分基础上叠加本地 BGE-M3 embedding rerank（1024 维，权重 65% builtin + 35% vector）
...
Builtin 模式总耗时 24 秒。Hybrid 模式受限于本地 embedding 推理速度，总耗时约 70 分钟。
...
| Question Type | Count | Builtin R@5 | + BGE-M3 R@5 | Builtin R@10 | + BGE-M3 R@10 | 提升 (R@5) |
|---|---|---|---|---|---|---|
| knowledge-update | 78 | 96.2% | **100.0%** | 100.0% | **100.0%** | +3.8pp |
| single-session-user | 70 | 92.9% | **98.6%** | 98.6% | **100.0%** | +5.7pp |
| multi-session | 133 | 95.5% | **97.7%** | 98.5% | **98.5%** | +2.2pp |
| single-session-assistant | 56 | 80.4% | **94.6%** | 85.7% | **94.6%** | **+14.2pp** |
| temporal-reasoning | 133 | 91.7% | **94.0%** | 94.0% | **97.7%** | +2.3pp |
| single-session-preference | 30 | 46.7% | **80.0%** | 73.3% | **86.7%** | **+33.3pp** |
...
- **single-session-preference（80.0% R@5）**：仍然是最弱的类别。尽管接入 BGE-M3 后提升了 33.3pp，这类问题的核心难点——间接偏好的词汇鸿沟——需要更高层次的语义理解或偏好提取机制。
- **temporal-reasoning（94.0% R@5）**：部分涉及"X 天前"等相对时间表达的问题需要数值推理，纯检索手段难以完全覆盖。
...
| Category | Count | Builtin R@5 | + BGE-M3 R@5 | Builtin R@10 | + BGE-M3 R@10 | 提升 (R@10) |
|---|---|---|---|---|---|---|
| adversarial | 841 | 89.7% | **92.9%** | 96.1% | **97.5%** | +1.4pp |
| temporal-inference | 446 | 90.1% | **92.4%** | 95.1% | **97.1%** | +2.0pp |
| single-hop | 282 | 75.5% | **82.6%** | 90.1% | **94.0%** | +3.9pp |
| temporal | 321 | 76.0% | **82.2%** | 85.4% | **91.6%** | +6.2pp |
| open-domain | 96 | 60.4% | **65.6%** | 70.8% | **74.0%** | +3.2pp |
...
上述弱项在接入 BGE-M3 后已经有显著改善（preference +33pp，assistant +14pp，temporal +6pp）。MarvMem 的架构还支持进一步提升：

1. **接入更强的 embedding 模型**（如 OpenAI text-embedding-3-large、Gemini embedding-001），预计在 open-domain 和 preference 类别上还有提升空间
2. **接入 LLM rerank**（如 Haiku、Gemini Flash），让语言模型对 top-K 候选做阅读理解级别的重排序
3. **调整 search weights 和 embedding weight**，针对具体使用场景优化各维度的权重比例
...
- Builtin 模式：无外部依赖
- Hybrid 模式：需要 OpenAI-compatible embedding 服务（如 LM Studio、Ollama、vLLM）
...
*Results verified April 2026. Builtin mode: zero external dependencies, zero API calls. Hybrid mode: local BGE-M3 embedding, zero remote API calls.*
